...
export async function middleware(request: NextRequest) {
  ...
  const publicRoutes = ["/", "/login", "/register", "/companies"];
  ...
  if (token && role) {
    ...
    if (
      role === "HRD" &&
      jobseekerRoutes.some((route) => pathname.startsWith(route))
    ) {
      return NextResponse.redirect(new URL("/hrd/dashboard", request.url));
    }

    // Prevent Society/Jobseeker from accessing HRD routes
    if (
      role === "Society" &&
      hrdRoutes.some((route) => pathname.startsWith(route))
    ) {
      return NextResponse.redirect(new URL("/jobseeker/jobs", request.url));
    }
